...
Adafruit_VL6180X *sensors[] = {&lox1, &lox2, &lox3, &lox4, &lox5};
const uint8_t COUNT_SENSORS = sizeof(sensors) / sizeof(sensors[0]);
const int sensor_gpios[COUNT_SENSORS] = {GPIO_LOX1, GPIO_LOX2, GPIO_LOX3, GPIO_LOX4, GPIO_LOX5}; // if any are < 0 will poll instead
...
void read_sensors() {
  uint8_t Range_1 = readSensor(lox1);
  uint8_t Range_2 = readSensor(lox2);
  uint8_t Range_3 = readSensor(lox3);
  uint8_t Range_4 = readSensor(lox4);
  uint8_t Range_5 = readSensor(lox5);
  ...
  Serial.println(Range_4);
  Serial.print("Range_5:");
  Serial.println(Range_5);
}
...
void timed_async_read_gpio() {
  ...
  digitalWrite(TIMING_PIN, HIGH);
  uint8_t pending_sensors = 0x1F; // binary 11111 → all 5 sensors pending
...
while (pending_sensors && ((millis() - start_time) < 1000)) {
  if ((pending_sensors & SENSOR1_MASK) && !digitalRead(GPIO_LOX1)) {
    range_lox1 = lox1.readRangeResult();
    status_lox1 = lox1.readRangeStatus();
    pending_sensors ^= SENSOR1_MASK;
  }
  if ((pending_sensors & SENSOR2_MASK) && !digitalRead(GPIO_LOX2)) {
    range_lox2 = lox2.readRangeResult();
    status_lox2 = lox2.readRangeStatus();
    pending_sensors ^= SENSOR2_MASK;
  }
  if ((pending_sensors & SENSOR3_MASK) && !digitalRead(GPIO_LOX3)) {
    range_lox3 = lox3.readRangeResult();
    status_lox3 = lox3.readRangeStatus();
    pending_sensors ^= SENSOR3_MASK;
  }
  if ((pending_sensors & SENSOR4_MASK) && !digitalRead(GPIO_LOX4)) {
    range_lox4 = lox4.readRangeResult();
    status_lox4 = lox4.readRangeStatus();
    pending_sensors ^= SENSOR4_MASK;
  }
  if ((pending_sensors & SENSOR5_MASK) && !digitalRead(GPIO_LOX5)) {
    range_lox5 = lox5.readRangeResult();
    status_lox5 = lox5.readRangeStatus();
    pending_sensors ^= SENSOR5_MASK;
  }
}
